...
def get_all_courses(sustc):
    # 专业内选课
    in_major_courses = get_url_courses(sustc,
                                       'http://jwxt.sustc.edu.cn/jsxsd/xsxkkc/xsxkKnjxk?kcxx=&skls=&skxq=&skjc=&sfym=false&sfct=false')

    # 跨专业选课
    cross_major_courses = get_url_courses(sustc,
                                          'http://jwxt.sustc.edu.cn/jsxsd/xsxkkc/xsxkFawxk?kcxx=&skls=&skxq=&skjc=&sfym=false&sfct=false')

    # 本学期选课计划
    this_term_courses = get_url_courses(sustc,
                                        'http://jwxt.sustc.edu.cn/jsxsd/xsxkkc/xsxkBxqjhxk?kcxx=&skls=&skxq=&skjc=&sfym=false&sfct=false')

    # 公选课计划
    public_courses = get_url_courses(sustc,
                                     'http://jwxt.sustc.edu.cn/jsxsd/xsxkkc/xsxkGgxxkxk?kcxx=&skls=&skxq=&skjc=&sfym=false&sfct=false&szjylb=')
...
if __name__ == '__main__':
    ...
    ob = SUSTech(usr, pwd, site)
    ob.login()
    # print(ob.get_home_page())
    grade_url = "http://jwxt.sustc.edu.cn/jsxsd/kscj/cjcx_list"
    rtext = ob.get_website(grade_url)
    soup = BeautifulSoup(rtext, 'lxml')
    table = soup.find_all('table', attrs={'id': True})
    table = table[0]
